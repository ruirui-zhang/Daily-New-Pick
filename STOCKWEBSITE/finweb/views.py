...
from finweb.models import UserProfile,Portfolio,Stockdata,Profitdata,Reportdata,recommenddata
...
def index(request):
    ...
    recommend = recommenddata.objects.filter()
    recommend_list = []
    name_list = []
    for i in range(6):
        stockdata = Stockdata.objects.filter(code=recommend[i].code)
        recommend_list.append([recommend[i].ii+1,recommend[i].code,stockdata[0].name])
    return render(request,'finweb/index.html',{'news_list1':news_list1,'news_list2':news_list2,'recommend_list':recommend_list})
...
def get_index_data(request):
    ...
    index_data =pro.get_k_data(ind,index=True)
...
def get_span_data(request):
    ...
    stockList = stockStr.split(str.encode('"'))
    stockList = stockList[1].split(str.encode(','))
    stockDic = {}
...
def stocks(request):
    ...
    if list and list[0]=='':
        ...
        # data =pro.get_realtime_quotes(list)
        ...
        for code in list:
            ...
            dic = {}
            dic['code'] = code
            dic['name'] = new_data['name']
            dic['price'] = round(float(new_data['now']),2)
            dic['open'] = round(float(new_data['open']),2)
            dic['pre_close'] = round(float(new_data['close']),2)
            dic['high'] = round(float(new_data['high']),2)
            dic['low'] = round(float(new_data['low']),2)
            dic['updown'] = round(float(new_data['now']) - float(new_data['close']),2)
...
def portfolio(request):
    ...
    for p in p_query:
        ...
        if p_list and p_list[0]== '':
            ...
            for pp in p_list:
                pattern = re.compile(r'^60*')
                match = pattern.match(pp)
                if match:
                    stockCode = 'sh'+ pp
                else:
                    stockCode = 'sz' + pp
                s_result=tmp_dict[stockCode]
                data_dic = {} #code nam pe pb ...
                data_dic['code'] = pp
                # if Stockdata.objects.filter(code=pp).exists():
                # s_result = Stockdata.objects.filter(code =pp)
                #如果此处查询出的值为空怎么办！！！！！！！！
                data_dic['name']= s_result['name']
                data_dic['pe'] = s_result['PE']
                data_dic['pb']= s_result['PB']
                data_dic['syd']= s_result['市盈(动)']
                data_dic['zsz']= s_result['总市值']
                # else:
                #     data_dic['pe'] = '--'
                #     data_dic['pb']= '--'
                #     data_dic['esp']= '--'
                ...
                data_dic['syj'] = s_result['市盈(静)']
                data_dic['cjl'] = s_result['成交量(手)']
                data_dic['cje'] = int(s_result['成交额(万)'])
                # else:
                #     data_dic['roe'] = '--'
                ...
                data_dic['turnover'] = s_result['turnover']
                # else:
                #     data_dic['income'] = '--'
...
def stockinfo(request):
    ...
    if request.method=='POST':
        ...
        sample = random.sample(range(Stockdata.objects.count()),1)
        print(sample[0])
    ...
    for p in port:
        ...
        dic['name'] = p.name
        portfolio_list.append(dic)
    # data =pro.get_realtime_quotes(code)
    quotation = easyquotation.use('tencent') # 新浪 ['sina'] 腾讯 ['tencent', 'qq']
    tmp_dict = quotation.market_snapshot(prefix=True)
    pattern = re.compile(r'^60*')
    match = pattern.match(code)
    ...
    stock['price'] = round(float(data['now']),2)
    stock['open'] = round(float(data['open']),2)
    stock['close'] = round(float(data['close']),2)
    stock['high'] = round(float(data['high']),2)
    stock['low'] = round(float(data['low']),2)
    stock['updown'] = round(float(data['now']) - float(data['close']),2)
    stock['percent'] = str(round(stock['updown']/stock['close']*100,2))+"%"
    stockCode =''
    pattern = re.compile(r'^60*')
    match = pattern.match(code)
    if match:
...
def get_info_stockdata(request):
    ...
    data = tmp_dict[code]
    # new_data = data[['code','now','high','low','close']]
    dic = {}
    dic['price'] = round(float(data['now']),2)
    dic['high'] = round(float(data['high']),2)
    dic['low'] = round(float(data['low']),2)
    dic['close'] = round(float(data['close']),2)
    dic['updown'] = round(float(data['now']) - float(data['close']),2)
    dic['percent'] = str(round(dic['updown']/dic['close']*100,2))+"%"
    if dic['high'] >= dic['close']:
        dic['high_red'] = True
    else:
        dic['high_red'] = False
    if dic['low'] >= dic['close']:
        dic['low_red'] = True
    else:
        dic['low_red'] = False
    return JsonResponse(dic,safe=False)
...
def get_info_kdata(request):
    ...
    # print type(code)
    # print "********\n"
    code_data = pro.daily(ts_code=str(code),start_date='20201223',end_date='20211223')
    # print "******** get_k_data\n"
    index_no_code = code_data.loc[:,['trade_date','open','close','high','low']]
    # print "******** index no code\n"
    data = index_no_code.to_json(orient="values")
    # print "******** to json\n"
    return JsonResponse(data,safe=False)
...
def get_macd(request):
    ...
    data = pro.get_k_data(code,start=fr,end=to)
...
#   query = Stockdata.objects.filter(code=code)
  ...
  params = urllib.parse.urlencode({'key':'f465968893a554b38238c7add53d5104','num':'50'})
...
#   else:
#       stockCode = code+'.SZ'
#   a = pro.anns(ts_code=stockCode,start_date='20210101',end_date='20220101')
  # print "error here??"
#   a = pro.anns()
  notices = []
  a=data['newslist']
...
def get_quant_data(request):
    ...
    for stock in stock_list:
        dic = {}
        # data =pro.get_realtime_quotes(stock)
        quotation = easyquotation.use('tencent') # 新浪 ['sina'] 腾讯 ['tencent', 'qq']
        tmp_dict = quotation.market_snapshot(prefix=True)
        pattern = re.compile(r'^60*')
        match = pattern.match(stock)
        if match:
            stockCode = 'sh'+ stock
        else:
            stockCode = 'sz' + stock
...
def get_stock_ratio(request):
    ...
    for stock in stock_list:
        ...
        data = pro.daily(ts_code=stockCode,start_date = start, end_date = end)
        d = data.loc[:,['trade_date','close']]
        close = d.set_index('trade_date')
        close.columns = [stock]
    ...
    print ("sharperesult!!!!!",sharpe_result,sharpe_ratio)
